...
module overmind::broker_it_yourself {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let state = State {
            ...
            create_offer_events: account::new_event_handle(admin),
            accept_offer_events: account::new_event_handle(admin),
            complete_transaction_events: account::new_event_handle(admin),
            release_funds_events: account::new_event_handle(admin),
            cancel_offer_events: account::new_event_handle(admin),
            open_dispute_events: account::new_event_handle(admin),
            resolve_dispute_events: account::new_event_handle(admin)
        };
        move_to(admin, state);
    }
    ...
    */
    public entry fun create_offer(
        ...
        if (!simple_map::contains_key(&state.creators_offers, &creator_address)) {
            let create_offer_ids = vector::singleton(next_offer_id);
            simple_map::add(&mut state.creators_offers, creator_address, create_offer_ids);
        } else {
            let creator_offer_ids = simple_map::borrow_mut(&mut state.creators_offers, &creator_address);
            vector::push_back(creator_offer_ids, next_offer_id);
        };
        
        if (offer.sell_apt) {
            assert_user_has_enough_funds<AptosCoin>(creator_address, offer.apt_amount);
            let resource_address = account::get_signer_capability_address(&state.cap);
            coin::transfer<AptosCoin>(creator, resource_address, offer.apt_amount);
        };

        let new_create_offer_event = broker_it_yourself_events::new_create_offer_event(
            state.offer_id,
            creator_address,
            arbiter,
            apt_amount,
        ...
        event::emit_event(&mut state.create_offer_events, new_create_offer_event);
    }
    ...
    */
    public entry fun accept_offer(user: &signer, offer_id: u128) acquires State {
        ...
        if (!offer.sell_apt) {
            ...
            let resource_address = account::get_signer_capability_address(&state.cap);
            coin::transfer<AptosCoin>(user, resource_address, offer.apt_amount);
        };

        let new_accept_offer_event = broker_it_yourself_events::new_accept_offer_event(
            offer_id,
            user_address,
            timestamp::now_seconds()
        );
        event::emit_event(&mut state.accept_offer_events, new_accept_offer_event);
    }
    ...
    */
    public entry fun complete_transaction(user: &signer, offer_id: u128) acquires State {
        ...
        let existing_offer = simple_map::borrow_mut(&mut state.offers, &offer_id);
        let offer = *existing_offer;
        let user_address = signer::address_of(user);
        ...
        if (offer.creator == user_address) {
            existing_offer.completion.creator = true;
        } else if (option::contains(&offer.counterparty, &user_address)) {
            existing_offer.completion.counterparty = true;
        };

        let new_complete_transaction_event = broker_it_yourself_events::new_complete_transaction_event(
            offer_id,
            user_address,
            timestamp::now_seconds() 
        );
        event::emit_event(&mut state.complete_transaction_events, new_complete_transaction_event);

        if (existing_offer.completion.creator && existing_offer.completion.counterparty) {
            simple_map::remove(&mut state.offers, &offer_id);
            remove_offer_from_creator_offers(&mut state.creators_offers, &user_address, &offer_id);
            
            let resource_signer = account::create_signer_with_capability(&state.cap);
            if (offer.sell_apt) {
                let counterparty_address = option::extract(&mut offer.counterparty);
                coin::transfer<AptosCoin>(&resource_signer, counterparty_address, offer.apt_amount);
            } else {
                coin::transfer<AptosCoin>(&resource_signer, offer.creator, offer.apt_amount);
            };

            let new_release_funds_event = broker_it_yourself_events::new_release_funds_event(
                offer_id,
                user_address,
                timestamp::now_seconds() 
            );
            event::emit_event(&mut state.release_funds_events, new_release_funds_event);
        }
    }
    ...
    */
    public entry fun cancel_offer(creator: &signer, offer_id: u128) acquires State {
        ...
        let new_cancel_offer_event = broker_it_yourself_events::new_cancel_offer_event(
            offer_id,
            timestamp::now_seconds()
        );
        event::emit_event(&mut state.cancel_offer_events, new_cancel_offer_event);
    }
    ...
    */
    public entry fun open_dispute(user: &signer, offer_id: u128) acquires State {
        ...
        let new_open_dispute_event = broker_it_yourself_events::new_open_dispute_event(
            offer_id,
            user_address,
            timestamp::now_seconds()
        );
        event::emit_event(&mut state.open_dispute_events, new_open_dispute_event);
    }
    ...
    */
    public entry fun resolve_dispute(
        ...
        let existing_offer = simple_map::borrow(&state.offers, &offer_id);
        let offer = *existing_offer;
        ...
        let new_resolve_dispute_event = broker_it_yourself_events::new_resolve_dispute_event(
            offer_id,
            transfer_to_creator,
            timestamp::now_seconds()
        );
        event::emit_event(&mut state.resolve_dispute_events, new_resolve_dispute_event);
    }
    ...
    */
    ...
    public fun get_available_offers(): SimpleMap<u128, Offer> acquires State {
        ...
        let state = borrow_global<State>(@admin);
        let offers_length = simple_map::length(&state.offers);
        let (ids, offer_values) = simple_map::to_vec_pair(state.offers);

        let index = 0;
        let available_offers = simple_map::create<u128, Offer>();
        loop {
            let offer = vector::borrow(&offer_values, index);
            if (option::is_none(&offer.counterparty)) {
                let id = vector::borrow(&ids, index);
                simple_map::add(&mut available_offers, *id, *offer);
            };

            index = index + 1;
            if (index >= offers_length) break;
        };
        available_offers
    }
    ...
    public fun get_arbitration_offers(): SimpleMap<u128, Offer> acquires State {
        ...
        loop {
            ...
            if (offer.dispute_opened) {
                let id = vector::borrow(&ids, index);
                simple_map::add(&mut arbitration_offers, *id, *offer);
            };
    ...
    public fun get_buy_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        loop {
            ...
            if (offer.creator == creator && !offer.sell_apt) {
                let id = vector::borrow(&ids, index);
                simple_map::add(&mut buy_offers, *id, *offer);
            };
    ...
    */
    ...
    public fun get_sell_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        loop {
            ...
            if (offer.creator == creator && offer.sell_apt) {
                let id = vector::borrow(&ids, index);
                simple_map::add(&mut sell_offers, *id, *offer);
            };
    ...
    public fun get_creator_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        loop {   
            ...
            if (offer.creator == creator) {
                let id = vector::borrow(&ids, index);
                simple_map::add(&mut creator_offers, *id, *offer);
            };
    ...
    */
    public(friend) inline fun remove_offer_from_creator_offers(
        ...
        let create_offer_ids = simple_map::borrow_mut(creators_offers, creator);
        let (_, index_offer_id) = vector::index_of(create_offer_ids, offer_id);
        vector::remove(create_offer_ids, index_offer_id);
    }
    ...
    */
    ...
    inline fun assert_user_has_enough_funds<CoinType>(user: address, coin_amount: u64) {
        ...
        let user_balance = coin::balance<CoinType>(user);
        assert!(user_balance >= coin_amount, ERROR_INSUFFICIENT_FUNDS);
    }
    ...
    inline fun assert_user_participates_in_transaction(user: address, offer: &Offer) {
        ...
        assert!(
            offer.creator == user || option::contains(&offer.counterparty, &user), 
